...
`15.0.0.0/24` subnet (check the virtual network configuration of the
host-only network assigned to a VM to gather this information), then the
`dhcp-range` option to enable a DHCP proxy could be: `dhcp-range=15.0.0.2,proxy`,
where `15.0.0.2` is the address assigned to the Docker host running the
